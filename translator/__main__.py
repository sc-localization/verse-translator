...
BACKENDS = ["claude", "gemini", "codex", "ollama", "lmstudio"]


def _build_backend(args: argparse.Namespace) -> TranslatorBackend:
    if args.backend == "claude":
        return ClaudeCLIBackend(model=args.model or "claude-haiku-4-5-20251001")
    if args.backend == "gemini":
        return GeminiCLIBackend(model=args.model or "gemini-2.5-flash")
    if args.backend == "codex":
        return CodexCLIBackend(model=args.model or "o4-mini")
    if args.backend == "ollama":
        return OllamaBackend(model=args.model or "qwen2.5:7b")
...
def main() -> None:
    ...
    parser.add_argument(
        "--backend", choices=BACKENDS, default=d.get("backend", "claude")
    )
    parser.add_argument("--model", default=None)
    parser.add_argument("--lmstudio-port", type=int, default=1234)
    parser.add_argument("--batch-size", type=int, default=d.get("batch_size", 50))
